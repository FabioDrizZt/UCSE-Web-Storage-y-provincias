...
if (localStorage.getItem('provincias')) {
  datos = JSON.parse(localStorage.getItem('provincias'))
} else {
  datos = getDatos()
  datos = datos.provincias
  localStorage.setItem('provincias', JSON.stringify(datos))
}

const provincias = document.querySelector('#grid-provincias')
datos.forEach((dato) => {
  // 1 - Creamos un article para cada provincia
  const article = document.createElement('article')
  // 2 - Le damos estilos al elemento
  article.classList.add('grid-item')
  // 3 - Agregamos un h2 con el nombre de la provincia
  article.innerHTML += `<h2> ${dato.nombre} </h2>`
  // 4 - Agregamos un evento cuando hagamos click
  article.addEventListener('click', () => {
    // 5 - Mostramos la información de la provincia
    location.href = `provincia.html?id=${dato.id}`
  })
  // 6 - Agregar el elemento creado a un nodo existente
  provincias.appendChild(article)
})
